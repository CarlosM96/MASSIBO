...
def main(arduino_port='/dev/ttyACM0',arduino_port2='/dev/ttyACM1'):
                ...
                for ch in channels:
                    if len(ch.waveform_data) > 0:
                        write_process = Process(
                            target=write_to_file_numpy,
                            args=(copy.deepcopy(ch), inicio, STORE_HEADER_COMMENT, run_counter, TYPE)
                        )
                        write_process.start()
