...
def multi_gaussian_fit(charges, num_peaks=5, plot=False, show_xtp=False, xlabel='Integrated Charge (ADU x samples)', title='Charge Histogram (Finger Plot) - DCR Trigger (Starts at 1 p.e.)', unit_label='(ADU * muestras)'):
    ...
    # --- Visualización XTP (Añadido) ---
    # Threshold at 1.5 p.e. (which is halfway between 1 p.e. and 2 p.e.)
...
def calculate_afterpulse_probability(timestamps, plot=False):
    """
...
def plot_results(acc, rej, t_sipm, t_leak):
    ...
    # 1. Templates Ideales
    ax[0].set_title("Ideal References (Synthetic)")
    ax[0].plot(x, t_sipm, 'b', linewidth=2, label='Ideal SiPM (Double Exp)')
    ax[0].plot(x, t_leak, 'r--', linewidth=2, label='Ideal Noise (Bipolar)')
    ax[0].legend()
...
def analyze_file(filepath, plot=False):
    """
    Función que orquesta el análisis de un archivo individual.
    Retorna (dcr, xtp, app) o (None, None, None) si falla.
    """
    ...
    # Convert waveforms to float32
    raw_waveforms_float = raw_waveforms.astype(np.float32)

    # --- Aplicar filtro de waveforms ---
    # Solo imprimimos mensajes si se solicita ploteo (debug mode) o si hay errores críticos
    if plot:
        print("\n--- Ejecutando filtro de waveforms ---")
    ...
    if len(accepted_waveforms) == 0:
        print(f"No pulses accepted in {os.path.basename(filepath)}")
        return None, None, None
    ...
    if not all_pulses:
        print(f"No pulses found in {os.path.basename(filepath)}")
    ...
    gain, pedestal = multi_gaussian_fit(all_charges, plot=plot)

    if gain is None:
        print("Failed to determine gain")
        return None, None, None

    # --- Amplitude Analysis for XTP ---
    ...
    if gain_amp is not None:
        ...
        crosstalk_prob = calculate_crosstalk_probability(amplitudes_val_pe)
        ...
        if len(non_burst_pulse_indices) > 0:
            ...
            crosstalk_prob_no_bursts = calculate_crosstalk_probability(amplitudes_val_pe_no_bursts)
        else:
            crosstalk_prob_no_bursts = 0.0
    else:
        crosstalk_prob = 0.0
        crosstalk_prob_no_bursts = 0.0

    # --- Afterpulsing Calculation ---
    ap_prob = calculate_afterpulse_probability(all_pulse_timestamps_s, plot=plot)
    
    # --- Cálculos Alternativos (Paper Definitions) ---
    XTP_alternative = 0.0
    APP_alternative = 0.0
    
    if len(non_burst_pulse_indices) > 0 and gain_amp is not None:
    ...
    if plot and len(non_burst_pulse_indices) > 0:
        if all_pulses_info:
    ...
    if plot:
        ...
        print(f"Total Waveforms: {len(raw_waveforms)}")
        print(f"Accepted Waveforms: {len(accepted_waveforms)}")
        print("-" * 20)
        print(f"DCR (No Bursts): {dcr * 1e3:.2f} mHz/mm²") 
        print(f"DCR (Raw/Total): {dcr_raw * 1e3:.2f} mHz/mm²")
        print("-" * 20)
        if gain_amp is not None:
            print(f"XTP : {crosstalk_prob * 100:.2f} %")
            print("-" * 20)
            print(f"APP : {APP_alternative * 100:.2f} %")
        print("="*30)

    return dcr, crosstalk_prob, ap_prob, APP_alternative, XTP_alternative
